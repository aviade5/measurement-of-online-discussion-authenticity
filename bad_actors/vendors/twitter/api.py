...
from vendors.twitter.status import Status
...
class Api(object):
    ...
    def __init__(self,
        ...
        self._number_of_api_requests = 0
        self.num_of_get_follower_ids_requests = 0
        self._num_of_get_friend_ids_requests = 0
        self.num_of_get_retweeter_ids_requests = 0
        self._num_of_get_users_requests = 0
        self.num_of_rate_limit_status = 0

        self._config_parser = getConfig()
        self._maximal_number_of_retrieved_users = int(
            self._config_parser.get("Twitter_Rest_Api", "maximal_number_of_retrieved_users"))
    ...
    def GetTweetById(self, tweet_id):
        url = "https://api.twitter.com/1.1/statuses/show/"+str(tweet_id)+".json"
        json = self._RequestUrl(url, 'GET', data={})
        data = self._ParseAndCheckTwitter(json.content)
        ...
        while True:
            if cursor:
                try:
                    ...
                    break
            json = self._RequestUrl(url, 'GET', data=parameters)
            self.num_of_get_retweeter_ids_requests += 1
            data = self._ParseAndCheckTwitter(json.content)
    ...
    def GetFriendIDs(self,
        ...
        while True:
            parameters['cursor'] = cursor
            json = self._RequestUrl(url, 'GET', data=parameters)
            self._num_of_get_friend_ids_requests += 1
            print("Number of GetFriendIds is: " + str(self._num_of_get_friend_ids_requests))
            logging.info("Number of GetFriendIds is: " + str(self._num_of_get_friend_ids_requests))
            try:
                data = self._ParseAndCheckTwitter(json.content)
                result += [x for x in data['ids']]
                if 'next_cursor' in data:
                ...
                print("---GetFriendIDs: Time to wait in seconds (Twitter-api.py) is: " + str(sec))
                logging.info("---GetFriendIDs: Time to wait in seconds (Twitter-api.py) is: " + str(sec))
                count_down_time(sec)
                num_of_friends = len(result)
                if num_of_friends >= self._maximal_number_of_retrieved_users:
                    break

                '''
                if sec is not 0:
                  friend_connection_type = unicode("friend")
                  self._db.create_and_save_author_connections(user_id, result, friend_connection_type)
                  sec = self.GetSleepTime('/friends/ids')
                  count_down_time(sec)
                  self._num_of_get_friend_ids_requests = 0
                ...
                sec = self.GetSleepTime('/friends/ids')
                print("---GetFriendIDs after exception ---> Time to wait in seconds (Twitter-api.py) is: " + str(sec))
                logging.info(
                    "--- GetFriendIDs after exception ---> Time to wait in seconds (Twitter-api.py) is: " + str(sec))
                count_down_time(sec)
                if e.message == unicode('Not authorized.'):
                    break
        return result
    ...
    def GetFollowerIDs(self,
        ...
        while True:
            ...
            parameters['cursor'] = cursor
            json = self._RequestUrl(url, 'GET', data=parameters)
            self.num_of_get_follower_ids_requests += 1
            print("Number of GetFollowerIds is: " + str(self.num_of_get_follower_ids_requests))
            logging.info("Number of GetFollowerIds is: " + str(self.num_of_get_follower_ids_requests))
            ...
            try:
                ...
                print("Time to wait in seconds (Twitter-api.py) is: " + str(sec))
                logging.info("Time to wait in seconds (Twitter-api.py) is: " + str(sec))
                count_down_time(sec)
                num_of_followers = len(result)
                if num_of_followers >= self._maximal_number_of_retrieved_users:
                    break
                '''
                if sec is not 0:
                  follower_connection_type = unicode("follower")
                  self._db.create_and_save_author_connections(user_id, result, follower_connection_type)
                  sec = self.GetSleepTime('/followers/ids')
                  #sec = sec + 100
                  count_down_time(sec)
                  self.num_of_get_follower_ids_requests = 0
                  if len(result) >= self._maximal_number_of_retrieved_users:
                    break
                '''
            except TwitterError as e:
                print(e.message)
                ...
                print("GetFollowerIDs after exception ---> Time to wait in seconds (Twitter-api.py) is: " + str(sec))
                logging.info(
                    "GetFollowerIDs after exception ---> Time to wait in seconds (Twitter-api.py) is: " + str(sec))
                count_down_time(sec)
    ...
    def GetFollowers(self,
        ...
        while True:
            ...
            self._number_of_api_requests = self._number_of_api_requests + 1
            print("Request number: " + str(self._number_of_api_requests))
            result += [User.NewFromJsonDict(x) for x in data['users']]
            ...
            print("Seconds to wait are: " + str(sec))
            time.sleep(sec)
    ...
    def UsersLookup(self,
        ...
        json = self._RequestUrl(url, 'GET', data=parameters)
        self._num_of_get_users_requests += 1
        print("Number of GetUsers requests is: " + str(self._num_of_get_users_requests))
        try:
            data = self._ParseAndCheckTwitter(json.content)
        except TwitterError, e:
            _, e, _ = sys.exc_info()
            t = e.args[0]
            if len(t) == 1 and ('code' in t[0]) and (t[0]['code'] == 34 or t[0]['code'] == 17):
                data = []
            else:
                raise
    ...
    def GetRateLimitStatus(self, resource_families=None):
        ...
        self.num_of_rate_limit_status += 1
        print("Number of rate limit status requests is: " + str(self.num_of_rate_limit_status))
        data = self._ParseAndCheckTwitter(json.content)
    ...
    ### created by Aviad
    def get_num_of_rate_limit_status(self):
        return self.num_of_rate_limit_status

    def get_num_of_get_users_requests(self):
        return self._num_of_get_users_requests
