...
class PyDictionary(object):
    ...
    def meaning(term):
        if len(term.split()) > 1:
            ...
            try:
                html = _get_soup_object("http://wordnetweb.princeton.edu/perl/webwn?s=" +
                    term)
                types = html.findAll("h3")
                if types[0].text == u'Your search did not return any results.':
                    return {'temp': None}
                if types[0].text == u'Sorry, your search can only contain letters, numbers, spaces, hyphens, periods, slashes, and/or apostrophes.':
                    return {'temp': None}
                length = len(types)
