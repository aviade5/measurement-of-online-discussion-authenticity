...
class Method_Executor(AbstractController):
    ...
    def _export_csv_files_for_statistics(self, output_folder_path, before_dict, after_dict, claims_dict):
        ...
        claim_ids = set(before_dict.keys() + after_dict.keys())
        for claim_id in claim_ids:
            ...
            claim_to_tweet_count_rows.append(
                (claim_id, claim_content, claim_keywords, snopes_publication_date, tweet_before_count, tweet_after_count, tweet_combined_count, total_tweet_count))
            claim_to_tweet_id_rows.append((claim_id, claim_content, claim_keywords, snopes_publication_date, ','.join(tweet_ids_before), ','.join(tweet_ids_after)))
